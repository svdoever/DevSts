...
$DevStsUsersJsonFile = Join-Path -Path $SiteFolder -ChildPath 'App_Data\DevStsUsers.json'

$DevStsZipPath = Join-Path -Path $PSScriptRoot -ChildPath DevSts.zip
if (!(Test-Path -Path $DevStsZipPath)) {
    Write-Warning "Expected DevSts zip file is missing at path '$DevStsZipPath'"
    Break
}
...
if (($DevStsWebSite -ne $null) -and ($DevStsWebSite.State -eq 'Started')) {
    "Stopping website '$SiteName'"
    Stop-Website -Name $SiteName
}

if (Test-Path -Path $SiteFolder) {
    "Deleting old StsDev folder '$SiteFolder'"
...
if ($DevStsUsersJson -ne $null) {
    ...
    new-item -force -path $DevStsUsersJsonFile -value $DevStsUsersJson -type file | Out-Null
}

if ($DevStsWebSite -eq $null) {
    "Creating website '$SiteName'"
    $DevStsWebSite = New-Website -Name $SiteName -HostHeader $SiteName -PhysicalPath $SiteFolder -ApplicationPool DefaultAppPool -Force
}

"Set right to folder '$SiteFolder' for application pool 'IIS AppPool\DefaultAppPool'"cmd /c icacls $SiteFolder /grant "IIS AppPool\DefaultAppPool:(OI)(CI)M" 
